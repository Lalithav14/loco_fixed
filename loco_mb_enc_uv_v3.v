`timescale 1ns/1ps
// loco_mb_enc_uv_v3.v — UV-channel encoder
// FIX: accumulator 512→128 bits (10-15x faster in iverilog)
// No S_SCAN2: k estimated from grad_sum (reliable for smooth chroma)
// Inter-block context for better boundary prediction
module loco_mb_enc_uv_v3 #(
    parameter MB_W   = 16,
    parameter MB_H   = 16,
    parameter BUDGET = 260
)(
    ...
    localparam MB_PIX      = MB_W * MB_H;  // 256
    localparam LOG2_MB_PIX = 8;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            col<=0;row<=0;prev_px<=0;px_cnt<=0;px_sum<=0;grad_sum<=0;
            scan1_prev<=8'd128;block_mean<=8'd128;acc<=0;acc_fill<=0;
            out_cnt<=0;overflow<=0;k_val<=0;state<=S_SCAN1;emit_idx<=0;
            hdr_phase<=0;pad_cnt<=0;content_sz<=0;enc_px<=0;emit_total<=0;
            ...
            S_SCAN1: begin
                ...
                if(s_axis_tvalid) begin
                    ...
                    px_sum<=px_sum+{10'b0,s_axis_tdata};
                    if(s_axis_tdata>=scan1_prev)
                        grad_sum<=grad_sum+{10'b0,s_axis_tdata-scan1_prev};
                    else
                        grad_sum<=grad_sum+{10'b0,scan1_prev-s_axis_tdata};
            ...
            S_MEAN: begin
                ...
                if      (grad_mean<10'd4)  k_val<=2'd0;
                else if (grad_mean<10'd12) k_val<=2'd1;
                else if (grad_mean<10'd40) k_val<=2'd2;
                else                       k_val<=2'd3;
                col<=0;row<=0;prev_px<=0;enc_px<=0;acc<=0;acc_fill<=0;
                out_cnt<=0;overflow<=0;state<=S_ENCODE;
            end
            S_ENCODE: begin
                if(acc_fill>=8'd8) begin
                    ...
                    if(col==MB_W-1) begin col<=0;row<=(row==MB_H-1)?4'd0:row+4'd1; end
                    else col<=col+4'd1;
                    mv=mapped9;quot=mv>>k_val;
