`timescale 1ns/1ps
// loco_mb_dec_v3.v
// KEY FIX: inter-block context ports added to match encoder.
// The encoder uses top_ctx_flat and left_ctx_px in its MED predictor.
// The decoder MUST use identical context or predictions diverge → decode errors.
// Since blocks are lossless, decoded pixels == original pixels, so
// out_bottom_flat and out_right_px can be fed directly to the next block.
// Works for both Y (32x32) and UV (16x16) via parameters.
module loco_mb_dec_v3 #(
    parameter MB_W   = 32,
    parameter MB_H   = 32,
    ...
    reg        is_raw;
    reg [1:0]  k_val;
    reg [10:0] comp_size,bytes_rd,px_cnt,drain_left;
    reg [7:0]  cur_byte;
    reg [2:0]  bit_pos;
    reg        have_byte;
    reg [7:0]  q_acc;
    ...
    reg [7:0]  prev_px;
    reg [8:0]  mp,full_rem,rc;
    reg [7:0]  mag,rn;
    reg        cb;

    // MED predictor — IDENTICAL to encoder context logic
    ...
    task emit_px;
        ...
        begin
            ...
            prev_px       <= px;
            line_buf[col] <= px;
            // capture context outputs from decoded (== original) pixels
            if (row==MB_H-1) out_bottom_flat[col*8+:8] <= px;
            if (col==MB_W-1) out_right_px <= px;
    ...
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            ...
            col<=0;row<=0;prev_px<=0;cur_byte<=0;bit_pos<=7;have_byte<=0;
            q_acc<=0;rem_cnt<=0;rem_acc<=0;rice_phase<=0;
            s_axis_tready<=1;m_axis_tvalid<=0;m_axis_tlast<=0;mb_done<=0;
        end else begin
                ...
                if(s_axis_tvalid) begin
                    case(hdr_cnt)
                        ...
                        2'd3:begin
                            ...
                            bytes_rd<=0;px_cnt<=0;q_acc<=0;rem_cnt<=0;
                            rem_acc<=0;rice_phase<=0;have_byte<=0;
                            if(is_raw) begin
                    ...
                    if(rice_phase==0) begin
                        if(cb==0) q_acc<=q_acc+8'd1;
                            ...
                            if(k_val==0) begin
                                ...
                                if(px_cnt==MB_PIX-10'd1) begin
                                    m_axis_tlast<=1;
                                    drain_left<=POST_HDR-comp_size;state<=S_DRAIN;
                                end
                            end else begin rice_phase<=1;rem_cnt<=0;rem_acc<=0; end
                        end
                    end else begin
            ...
            S_DONE: begin
                mb_done<=1;state<=S_HDR;hdr_cnt<=0;col<=0;row<=0;prev_px<=0;
            end
            endcase
        end
    end
