`timescale 1ns/1ps
// tb_quick_5mp.v — tests 3 Y + 2 UV blocks from a 5MP frame
// Identical logic to tb_quick_v3.v but reads frame_y_5mp.hex etc.
// Run this first to verify compression ratios on 5MP content.
// 5MP blocks: Y=81 cols x 61 rows. Blocks 0,1,2 are row0,col0/1/2.
module tb_quick_5mp;
...
loco_mb_enc_y_v3 #(.MB_W(32),.MB_H(32),.BUDGET(1028)) YENC(
    ...
    .top_ctx_valid(y_etop_v),.top_ctx_flat(y_etop),
...
loco_mb_dec_v3 #(.MB_W(32),.MB_H(32),.BUDGET(1028)) YDEC(
    ...
    .top_ctx_valid(y_dtop_v),.top_ctx_flat(y_dtop),
...
loco_mb_enc_uv_v3 #(.MB_W(16),.MB_H(16),.BUDGET(260)) UVENC(
    ...
    .top_ctx_valid(uv_etop_v),.top_ctx_flat(uv_etop),
    .left_ctx_valid(uv_eleft_v),.left_ctx_px(uv_eleft),
...
task run_y; input integer blk; begin
    ...
    timeout=0; while(!y_enc_done&&timeout<100000)begin @(posedge clk);timeout=timeout+1;end
    y_etop<=y_enc_bottom; y_etop_v<=1; y_eleft<=y_enc_right; y_eleft_v<=1;
    @(posedge clk);
    $display("  enc: %0dB k=%0d ratio=%0d%%",y_enc_bytes,y_pkt[0][6:5],100-100*y_enc_bytes/1024);
    ...
    timeout=0; while(!y_dec_done&&timeout<100000)begin @(posedge clk);timeout=timeout+1;end
    repeat(4)@(posedge clk);
    y_dtop<=y_dec_bottom; y_dtop_v<=1; y_dleft<=y_dec_right; y_dleft_v<=1;
    errors=0;
    for(i=0;i<1024;i=i+1) if(y_dec[i]!==y_frame[blk*1024+i]) errors=errors+1;
    if(errors==0) $display("  LOSSLESS");
    else $display("  ERRORS: %0d",errors);
end endtask

task run_uv; input integer blk; begin
    ...
    timeout=0; while(!uv_enc_done&&timeout<50000)begin @(posedge clk);timeout=timeout+1;end
    uv_etop<=uv_enc_bottom; uv_etop_v<=1; uv_eleft<=uv_enc_right; uv_eleft_v<=1;
    @(posedge clk);
    $display("  enc: %0dB k=%0d ratio=%0d%%",uv_enc_bytes,uv_pkt[0][6:5],100-100*uv_enc_bytes/256);
    ...
    timeout=0; while(!uv_dec_done&&timeout<50000)begin @(posedge clk);timeout=timeout+1;end
    repeat(4)@(posedge clk);
    uv_dtop<=uv_dec_bottom; uv_dtop_v<=1; uv_dleft<=uv_dec_right; uv_dleft_v<=1;
    errors=0;
    for(i=0;i<256;i=i+1) if(uv_dec[i]!==u_frame[blk*256+i]) errors=errors+1;
    if(errors==0) $display("  LOSSLESS");
    else $display("  ERRORS: %0d",errors);
end endtask

initial begin
    $readmemh("frame_y_5mp.hex",y_frame,0,3*1024-1);
    $readmemh("frame_u_5mp.hex",u_frame,0,2*256-1);
    $display("=== QUICK TEST — 5MP CONTENT ===");
    rst_n=0; repeat(8)@(posedge clk); rst_n=1; repeat(4)@(posedge clk);
    run_y(0); run_y(1); run_y(2);
    rst_n=0; repeat(4)@(posedge clk); rst_n=1;
    uv_etop_v=0; uv_dtop_v=0; uv_eleft_v=0; uv_dleft_v=0;
    repeat(4)@(posedge clk);
    run_uv(0); run_uv(1);
    $display("=== DONE ===");
    $finish;
