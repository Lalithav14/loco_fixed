...
// loco_mb_dec_v2.v
// ─────────────────────────────────────────────────────────────────────────────
// FUNCTIONALLY IDENTICAL to loco_mb_dec.v (v1).
// The decoder RTL was always correct. All 73,771 errors in the original
// full-frame test were caused by the TESTBENCH not honouring s_axis_tready,
// not by any bug in this module.
//
// CRITICAL USAGE RULE (must be followed in every testbench/wrapper):
//   Before feeding each byte to s_axis_tdata, ALWAYS wait for s_axis_tready=1.
//   In S_RICE state the decoder takes ~9 clocks per byte (1 load + 8 bit-process).
//   Skipping this wait causes every 8th byte to be dropped silently.
//   Correct pattern:
//       while (!s_axis_tready) @(posedge clk);
//       s_axis_tvalid = 1; s_axis_tdata = pkt[j];
//
// Versioned as v2 so it can be used alongside v1 without filename conflicts.
// ─────────────────────────────────────────────────────────────────────────────
module loco_mb_dec_v2 #(
    parameter MB_W   = 32,
    parameter MB_H   = 32,
