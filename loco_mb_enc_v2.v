`timescale 1ns/1ps
// loco_mb_enc_v2.v  — Drop-in improvement over loco_mb_enc.v
// Changes:
//   1. S_SCAN2 removed: k estimated from grad_sum in S_SCAN1 (saves 1025 clk/Y block)
//   2. Inter-block context ports: top_ctx_flat, left_ctx_px fix MED predictor at boundaries
//   3. Context output ports: out_bottom_flat, out_right_px for chaining
// Use loco_ctx_wrap_v2 to manage context automatically.
module loco_mb_enc_v2 #(
    parameter MB_W   = 32,
    parameter MB_H   = 32,
    ...
    // inter-block context in  (tie valid=0 when not available)
    ...
    // context outputs for chaining to neighbours
    ...
    localparam S_SCAN1=3'd0, S_MEAN=3'd1, S_ENCODE=3'd2,
               S_FLUSH=3'd3, S_HDR=3'd4, S_EMIT=3'd5, S_PAD=3'd6, S_DONE=3'd7;

    reg [4:0] col, row;
    reg [7:0] line_buf [0:MB_W-1];
    reg [7:0] prev_px, block_mean;

    // MED predictor with inter-block context
    ...
    wire [7:0] ctx_c = (row==0||col==0)
                       ? ((row==0&&col>0&&top_ctx_valid) ? top_ctx_flat[(col-1)*8 +: 8] : block_mean)
                       : line_buf[col-1];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            px_sum<=0; grad_sum<=0; scan1_prev<=8'd128; block_mean<=8'd128;
            ...
            S_SCAN1: begin
                ...
                if (s_axis_tvalid) begin
                    ...
                    // gradient: |px - prev|, free at same time as scan1
                    if (s_axis_tdata >= scan1_prev)
                        grad_sum <= grad_sum + {12'b0, s_axis_tdata - scan1_prev};
                    else
                        grad_sum <= grad_sum + {12'b0, scan1_prev - s_axis_tdata};
                    scan1_prev <= s_axis_tdata;
                    px_cnt     <= px_cnt + 11'd1;
                    if (px_cnt==MB_PIX-1) begin s_axis_tready<=0; state<=S_MEAN; end
                end
            end

            // 1 clock — replaces S_MEAN+S_SCAN2+S_PICKK from v1 (saves 1027 clk)
            // k from gradient mean: raw gradients ~4x MED residuals on natural images
            // so thresholds are 4x the original scan2 thresholds
            S_MEAN: begin
                block_mean <= px_sum[LOG2_MB_PIX+7:LOG2_MB_PIX];
                grad_mean   = grad_sum >> LOG2_MB_PIX;
                if      (grad_mean < 11'd8)   k_val<=2'd0;
                else if (grad_mean < 11'd32)  k_val<=2'd1;
                else if (grad_mean < 11'd128) k_val<=2'd2;
                else                          k_val<=2'd3;
            ...
            S_ENCODE: begin
                if (acc_fill>=10'd8) begin
                    if (!overflow&&out_cnt<BUDGET) begin
                        out_buf[out_cnt]<=acc[511:504]; out_cnt<=out_cnt+11'd1;
                    end
                    if (out_cnt>=(BUDGET-2)) overflow<=1;
                    acc<=acc<<8; acc_fill<=acc_fill-10'd8;
                    ...
                    // Rice encode
                    mv=mapped9; quot=mv>>k_val; remainder=mv&((9'd1<<k_val)-9'd1);
